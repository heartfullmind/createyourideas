...
export class IdeaComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.ideaService
      ...
      .subscribe((res: HttpResponse<IIdea[]>) => this.paginateIdeas(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IIdea) {
    return item.id;
  }
  ...
  protected paginateIdeas(data: IIdea[], headers: HttpHeaders) {
